...
char * app_my_rank_str(void) {
  sl_wisun_rpl_info_t rpl_info;
  sl_status_t ret;
  
  ret = sl_wisun_get_rpl_info(&rpl_info);
  if (ret == SL_STATUS_OK) {
    snprintf(json_string, 1024, 
      "{\n"
      "  \"dodag_rank\": %u,\n"
      ...
      "}",
      rpl_info.dodag_rank,
      rpl_info.instance_id
    );
  } else {
    snprintf(json_string, 1024, "{\"error\": \"Failed to get RPL info\", \"status\": \"0x%04x\"}", (uint16_t)ret);
  }
  return json_string;
}
