...
sl_wisun_coap_packet_t *coap_callback_board(
    const sl_wisun_coap_packet_t *const req_packet)
{
  snprintf(coap_response, COAP_MAX_RESPONSE_LEN, "%s", "FG28");
  return app_coap_reply(coap_response, req_packet);
}
...
#define JSON_ALL_DATA_FORMAT_STR \
  ...
  // Read all sensor data on-demand with delays to prevent I2C bus contention
  read_meter_data();
  sl_sleeptimer_delay_millisecond(200); // Allow I2C bus to settle between sensors

  read_si7021_data();
  sl_sleeptimer_delay_millisecond(300); // Longer delay before VEML6035 (needs measurement cycle time)
...
void read_si7021_data(void)
{
  ...
  // Add small delay before I2C read to ensure bus recovery
  sl_sleeptimer_delay_millisecond(50);

  // Retry logic with exponential backoff for I2C reliability
  ...
  for (uint8_t retry = 0; retry < max_retries; retry++)
  {
    ...
    // Retry failed - add delay before next attempt
    if (retry < max_retries - 1)
    {
      printf("WARNING: Si7021 read failed (status: 0x%04lX), retry %d/%d...\n",
             status, retry + 1, max_retries);
      sl_sleeptimer_delay_millisecond(100 * (retry + 1)); // Exponential backoff: 100ms, 200ms
    }
  }

  // All retries failed
  printf("ERROR: Failed to read Si7021 sensor after %d retries! Final status: 0x%04lX\n",
         max_retries, status);
...
void read_veml_data(void)
{
  ...
  // CRITICAL: VEML6035 has ~100ms integration time per channel
  // Must wait for complete measurement cycle before attempting read
  sl_sleeptimer_delay_millisecond(250); // Increased from 150ms to 250ms for full cycle

  // Retry logic with exponential backoff for I2C reliability
  const uint8_t max_retries = 5; // Increased from 3 to 5

  // Read ALS with retries
  for (uint8_t retry = 0; retry < max_retries; retry++)
  {
    status = sl_veml6035_get_als_lux(sl_i2cspm_sensor, &als);

    if (status == SL_STATUS_OK)
    {
      break; // Success - continue to white reading
    }

    if (retry < max_retries - 1)
    {
      printf("WARNING: VEML6035 ALS read failed (status: 0x%04lX), retry %d/%d...\n",
             status, retry + 1, max_retries);
      sl_sleeptimer_delay_millisecond(150 * (retry + 1)); // Increased backoff: 150ms, 300ms, 450ms, 600ms
    }
  }

  if (status != SL_STATUS_OK)
  {
    printf("ERROR: Failed to read ALS from VEML6035 after %d retries! Status: 0x%04lx\n",
           max_retries, status);
    printf("Sensor appears to be disconnected. Marking as unavailable.\n");
  ...
  // CRITICAL: Must wait for WHITE channel measurement cycle after ALS read
  // Each channel requires separate integration time (~100ms each)
  sl_sleeptimer_delay_millisecond(150); // Increased from 100ms to 150ms

  // Read WHITE with retries
  for (uint8_t retry = 0; retry < max_retries; retry++)
  {
    ...
    if (retry < max_retries - 1)
    {
      printf("WARNING: VEML6035 WHITE read failed (status: 0x%04lX), retry %d/%d...\n",
             status, retry + 1, max_retries);
      sl_sleeptimer_delay_millisecond(150 * (retry + 1)); // Increased backoff: 150ms, 300ms, 450ms, 600ms
    }
  }
  ...
  if (status != SL_STATUS_OK)
  {
    printf("ERROR: Failed to read WHITE from VEML6035 after %d retries! Status: 0x%04lx\n",
           max_retries, status);
    printf("Sensor appears to be disconnected. Marking as unavailable.\n");
    latest_veml_data.available = false;
    latest_veml_data.als = 0;
...
void init_si7021_sensor(void)
{
  ...
  else
  {
    ...
    // Si7021 needs time for internal configuration after init
    printf("Waiting for Si7021 to stabilize...\n");
    sl_sleeptimer_delay_millisecond(300); // Give sensor time to settle

    // Perform initial test read with retry logic
    uint32_t rh_test;
    int32_t temp_test;
    ...
    for (uint8_t retry = 0; retry < 3; retry++)
    {
      ...
      {
        printf("WARNING: Could not read Si7021: 0x%04lX (retry %d/3)\n", status, retry + 1);
        if (retry < 2)
        {
          sl_sleeptimer_delay_millisecond(200); // Wait before retry
        }
      }
    }
    ...
    if (!read_success)
    {
      printf("NOTE: Initial test read failed, but sensor may work after additional stabilization time\n");
    }
  }
  printf("=== Si7021 Initialization Complete ===\n\n");

  // Allow I2C bus and sensor time to fully stabilize after initialization
...
void init_veml6035_sensor(void)
{
  ...
  else
  {
    ...
    // CRITICAL: Configure integration time explicitly for predictable timing
    // Integration time = time sensor needs to complete ONE measurement per channel
    printf("Configuring integration time to 100ms...\n");
    status = sl_veml6035_configure_integration_time(sl_i2cspm_sensor, SL_VEML6035_ALS_IT_100MS);
    if (status != SL_STATUS_OK)
    {
      printf("WARNING: Failed to configure integration time! Status: 0x%04lx\n", status);
    }

    // CRITICAL: After configuring integration time, sensor starts NEW measurement cycles
    // Must wait for BOTH channels (ALS, WHITE) to complete their first measurements
    // With 100ms IT: 100ms ALS + 100ms WHITE + 200ms extra settling = 400ms total
    printf("Waiting for sensor to complete first measurement cycles...\n");
    printf("(ALS 100ms + WHITE 100ms + settling 200ms = 400ms total)\n");
    ...
    for (uint8_t retry = 0; retry < 5; retry++) // Increased retries from 3 to 5
    {
      status = sl_veml6035_get_als_lux(sl_i2cspm_sensor, &als_test);
      if (status == SL_STATUS_OK)
      {
        sl_sleeptimer_delay_millisecond(100); // Delay between ALS and WHITE
        ...
        {
          printf("WARNING: WHITE channel read failed: 0x%04lx (attempt %d/5)\n", status, retry + 1);
        }
      }
      else
      {
        printf("WARNING: ALS channel read failed: 0x%04lx (attempt %d/5)\n", status, retry + 1);
      }

      if (retry < 4)
      {
        printf("        Waiting 300ms before retry...\n");
        sl_sleeptimer_delay_millisecond(300); // Increased from 200ms to 300ms
      }
    }

    if (!read_success)
    {
      printf("WARNING: Initial test read failed after 5 attempts\n");
      printf("         Sensor will still be marked as 'available'\n");
      printf("         CoAP reads will retry with longer delays\n");
    }
  }
  printf("=== VEML6035 Initialization Complete ===\n\n");

  // Allow even more time for sensor to fully stabilize
  sl_sleeptimer_delay_millisecond(300); // Additional 300ms
}
