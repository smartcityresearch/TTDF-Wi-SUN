    ...
    const sl_wisun_coap_packet_t *const req_packet)
{
  ...
  snprintf(coap_response, COAP_MAX_RESPONSE_LEN, "%u", rpl_rank);
  return app_coap_reply(coap_response, req_packet);
}
  ...
  if (req_packet->payload_len)
  {
    ...
    if (payload_str != NULL)
    {
      ...
      if (res == 2)
      {
        ...
        set_get_res = set_app_parameter(parameter_name, app_parameters.network_index, value, coap_response);
      }
      else
      {
        ...
        if (res == 1)
        {
          ...
          set_get_res = get_app_parameter(parameter_name, app_parameters.network_index, &value, coap_response);
        }
      }
      // Free payload_str
      sl_wisun_coap_free(payload_str);
