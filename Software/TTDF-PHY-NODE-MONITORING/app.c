...
#define PARENT_INFO_ITEMS       \
  parent_tag,                   \
      ...
      parent_info.rsl_in,       \
      parent_info.rsl_out //  parent_info.lifetime
                          //  secondary_tag,
...
void app_task(void *args)
{
  ...
  // Initialize Relay Status LED to OFF state (relay OFF for active-low configuration)
  // This ensures relay starts OFF before any network connection
  printf("\n=== Initializing Relay Status LED (Relay Control) ===\n");
  sl_led_turn_off(&sl_led_relay_status); // Turn Relay Status LED ON = Relay OFF (active-low)
  printf("Relay Status LED initialized: OFF (Relay OFF - active HIGH)\n");
  ...
  { // Only FFNs support Direct Connect
  ...
  if (network[app_parameters.network_index].set_leaf)
  {
    if (network[app_parameters.network_index].device_type == SL_WISUN_ROUTER)
    {
      sl_wisun_set_leaf(true);
      printfBothTime("LEAF mode %d\n", network[app_parameters.network_index].set_leaf);
    }
    else
    {
      printfBothTime("LEAF mode %d is incompatible with device_type %d\n",
                     network[app_parameters.network_index].set_leaf, network[app_parameters.network_index].device_type);
    }
  }
  printfBothTime("app_parameters.auto_send_sec %d\n", app_parameters.auto_send_sec);

  sl_wisun_set_preferred_pan(network[app_parameters.network_index].preferred_pan_id);
  printfBothTime("preferred_pan_id 0x%04x\n", network[app_parameters.network_index].preferred_pan_id);
  ...
  *******************************************/
  ...
  while (1)
  {
    ...
    if (join_state == SL_WISUN_JOIN_STATE_OPERATIONAL)
    {
      ...
      // Sensor data is now read on-demand when CoAP endpoints are hit
      // (removed periodic updates)
...
sl_wisun_mac_address_t _get_parent_mac_address_and_update_parent_info(void)
{
  ...
  if (ret)
    printf("[Failed: sl_wisun_get_neighbors() returned 0x%04x]\n", (uint16_t)ret);
...
sl_status_t _select_destinations(void)
{
  ...
  sl_wisun_stoip6(network[app_parameters.network_index].udp_notification_dest, strlen(network[app_parameters.network_index].udp_notification_dest), udp_notification_sockaddr_in6.sin6_addr.address);
  ...
  sl_wisun_stoip6(network[app_parameters.network_index].coap_notification_dest, strlen(network[app_parameters.network_index].coap_notification_dest), coap_notification_sockaddr_in6.sin6_addr.address);
                                ...
                                bool to_console, bool to_rtt, bool to_udp, bool to_coap)
{
  ...
  if (to_coap == true)
  { // Send to CoAP notification port
